...
#v.1 
"""
I have finished v.1 tool of compare BCU and BIOS Release-Note.
There are its functions:
...
Note1 : The BCU.txt / BIOS Release_Note.xlsm must exist and do not opening. The result.xlsm can not opening either(Maybe you run once).
Note2: Put .exe in same floder in pre_release.
Note3. Support after G5(Before G4, Release Note is word.docx).

Attached is  .py and .exe.
\\10.242.149.15\SW1-HP_AIO_bios\CMIT_BIOS\Tools\compare_BCU_RN\compare_BCU_RN_code_exe_v2.7z
Following link is the result of some platform.*
\\10.242.149.15\SW1-HP_AIO_bios\CMIT_BIOS\Tools\compare_BCU_RN\compare_BCU_RN_for_zip.7z 

How to use :
1.	Put .exe in your pre_release floder.
2.	Put {any}BCU.txt or {any}bcu.txt in your pre_release floder.
3.	You can put {any}_SHA256.txt or Custom Test Suite.log / amdz in too(This step skip is FINE).
...
To do (add function) list :
1.  Try to get external-link.
2.  Remove un-released file after compare. EX: amdz.txt SHA256.txt …
3.  Check more platform.
4.  Customize Build-Date/PI/GbE… of each different for different format.
5.  Customize PD FW of each different for different format (Now show all).
"""
############################
...
if not excelName : #empty
    ...
    if platform in AMDPlatformDict :
        ...
        isAMDPlatform = False
    if platform in AMIPlatformDict :
        isAMIPlatform = True 
    else : 
        isAMIPlatform = False
...
#print(bcu_content)
#print(bcu_content.index("System BIOS Version\n")+1)
#print(rRowInfoName)
            ...
            try :
                ...
                if i == "System BIOS Version" :
                    ...
                    if len(me_name) > 0 :
                        ...
                        print("\nCan not get .\\METools\\FWUpdate\\MEFW\\ME_{version}.bin !")
                    outputFile[0].at[i, "Reference Info"] = mef
                    continue
                elif i == "GbE Version" :
                    AMIbinFile = ""
                    ...
                    pass
                    #print("Something AMI do not handle now!")
                    #print(i)
            except :
                pass
